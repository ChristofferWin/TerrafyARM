...
type HtmlObject struct {
	...
	CountOfArmResourceTypes int    `CountOfArmResourceTypes`
}
...
type StatusObject struct {
	...
	CountUniqueARMResourceTypes    int     `json:CountUniqueResourceTypes`
...
Install Terraform (Linux / MacOS) visit: https://developer.hashicorp.com/terraform/tutorials/aws-get-started/install-cli`))
	...
	CheckOnlineVersion(*chromeCustomPath)
	...
	if len(verifiedFiles) == 0 {
		convertPathSymbols := strings.Replace(*filePath, "\\", "/", -1)
		if strings.Contains(convertPathSymbols, ".json") {
			logFatal(fmt.Sprintf("The ARM template provided '%s' is either in a wrong json format or missing the json object 'properties'\nFor more information see => %s", strings.Split(convertPathSymbols, "/")[len(strings.Split(convertPathSymbols, "/"))-1], "https://github.com/ChristofferWin/TerrafyARM/tree/main/docs/arm-templates-examples"))
		} else {
			logFatal(fmt.Sprintf("All ARM templates located at '%s' are not valid, either because they are in the wrong json format or missing the json object 'properties\nFor more information see => %s", *filePath, "https://github.com/ChristofferWin/TerrafyARM/tree/main/docs/arm-templates-examples"))
		}
		return
	}
	...
	ReturnStatusObject.CountUniqueARMResourceTypes = len(sortedResourceTypes)
		...
		for index, resourceType := range sortedResourceTypes {
			...
			if resourceType != "" {
				...
				if rawHtml != "not_found" && rawHtml != "" {
					...
					cleanHtml = SortRawHtml(rawHtml, resourceType, logTime, len(sortedResourceTypes))
	...
	if len(htmlObjectsFromCache) != 0 && len(htmlObjects) > len(htmlObjectsFromCache) {
		if !*noCache {
			fmt.Println(htmlObjects[0].CountOfArmResourceTypes, len(sortedResourceTypes))
			logFatal("TerrafyARM has detected that new resource types have been added to the file/folder with ARM templates\nPlease rerun the command twice: first with the flag -clear-cache, and then without it to update the cache\nUse 'TerrafyARM -help' for more information")
		}
	}
	...
	if len(terraformObject.CompileObjects) == 0 {
		convertPathSymbols := strings.Replace(*filePath, "\\", "/", -1)
		if strings.Contains(convertPathSymbols, ".json") {
			logFatal(fmt.Sprintf("The ARM template provided '%s' did not match any 'AzureRM' Terraform provider attributes - An issue can always be raised at => %s", *filePath, "https://github.com/ChristofferWin/TerrafyARM/issues"))
		} else {
			logFatal(fmt.Sprintf("All ARM templates located at '%s' did not match any 'AzureRM' Terraform provider attributes - An issue can always be raised at => %s", *filePath, "https://github.com/ChristofferWin/TerrafyARM/issues"))
		}
	}
	...
	if ReturnStatusObject.CountFilesCreated > 0 {
...
func CheckOnlineVersion(customChromePath string) {
	HtmlBody := ""
	url := "https://github.com/ChristofferWin/TerrafyARM/releases/"
	...
	logVerbose("Checking for new versions online")

	err := chromedp.Run(taskCtx,
		chromedp.Navigate(string(url)),
		chromedp.WaitReady("body"),                   // Wait for the body to be ready
		chromedp.Sleep(time.Duration(2)*time.Second), // Give some time for JS to execute
		chromedp.InnerHTML("*", &HtmlBody, chromedp.ByQueryAll),
	)
	if err != nil {
		logWarning("'TerrafyARM' was not able to check for any updates, continuing...")
		return
	}

	splitHtml := strings.Split(HtmlBody, "\n")
	newestVersionLine := ""
	for index, line := range splitHtml {
		if strings.Contains(line, `a href="/ChristofferWin/TerrafyARM/releases/latest`) {
			newestVersionLine = splitHtml[index-2]
			break
		}
	}
	newestVersion := ""
	splitNewestVersionLine := strings.Split(newestVersionLine, " ")
	for _, line := range splitNewestVersionLine {
		if strings.Contains(line, `ChristofferWin/TerrafyARM/releases/tag/`) {
			newestVersion = strings.TrimSuffix(strings.Split(line, "/")[len(strings.Split(line, "/"))-1], `"`)
		}
	}

	if CurrentVersion != newestVersion {
		logWarning(fmt.Sprintf("A new version '%s' of 'TerrafyARM' is ready for download, consider updating...", newestVersion))
		logVerbose("While 'TerrafyARM' is in its early stages, you are highly encouraged to update the application right away, simply because improvements may come rapidly")
	}
...
func NewTerraformConfig(terraformCompiledObject CompileObject, seperatedResources bool) string {
	...
	/*
		if terraformCompiledObject.ResourceName == "ruc-we-corenetwork-vnet01-to-ruc-we-ipaasintegrations-peer" {
			for _, block := range terraformCompiledObject.BlockAttributes {
				...
				for index, rootAttribute := range block.RootAttribute {
					fmt.Println(index, "name", rootAttribute.Name, "is", rootAttribute.IsBlock, rootAttribute.BlockName, rootAttribute.UniqueBlockName, rootAttribute.Value)
				}
			}
		}
	*/
	...
	returnRootTerraformStringConfig := ""
	if len(returnRootTerraformConfig) > 0 {
		if len(returnRootTerraformConfig) == 5 {
			match := false
			for index, line := range returnRootTerraformConfig {
				if index != 0 && len(line) > 1 && !strings.Contains(line, "provider") && !strings.Contains(line, "}") {
					match = true
				}
			}
			if match {
				returnRootTerraformStringConfig = strings.Join(returnRootTerraformConfig, "\n")
			}
		} else {
			returnRootTerraformStringConfig = strings.Join(returnRootTerraformConfig, "\n")
		}
		return returnRootTerraformStringConfig
	}
	return ""
}
	...
	for _, terraformAttribute := range terraformAttributesNotFound {
		for _, attribute := range htmlObject.Attribute {
			if terraformAttribute.Name == attribute.Parent && attribute.Type != "armObject" && attribute.Required {
				rootAttribute := RootAttribute{
					Name:      attribute.Name,
					Value:     fmt.Sprintf("value from ARM NOT FOUND BUT REQUIRED, see link for help (TOP OF THE PAGE) => \\%s", attribute.TerraformURL),
			...
			for _, requiredAttribute := range allRequiredAttributes {
				if name == requiredAttribute.Name && requiredAttribute.Type != "armObject" {
					rootAttribute := RootAttribute{
						...
						Value:           fmt.Sprintf("value from ARM NOT FOUND BUT REQUIRED, see link for help (TOP OF THE PAGE) => \\%s", requiredAttribute.TerraformURL),
	...
	if terraformBlock != "" {
		if terraformAttribute != (RootAttribute{}) {
			if valueTypeName == "bool" {
				...
				return fmt.Sprintf("%s\n%s = %s", terraformBlock, terraformAttribute.Name, fmt.Sprintf("%.2f", terraformAttribute.Value.(float64)))
			} else {
		...
		case map[string]interface{}:
			{
				...
				} else if CheckForString(jsonMap["type"]) {
					if strings.ToLower(jsonMap["type"].(string)) == "microsoft.compute/virtualmachines" {
						specialResourceType = GetArmWebAndComputeKind(jsonMap["properties"])
					}
				}

				if CheckForString(jsonMap["type"]) && CheckForString(jsonMap["name"]) && CheckForString(jsonMap["id"]) {
					armObject := ArmObject{
	...
	*/
	...
	for x := 1; x < 25; x++ {
		err := chromedp.Run(taskCtx,
			chromedp.Navigate(string(url)),
			chromedp.WaitReady("body"),                   // Wait for the body to be ready
			chromedp.Sleep(time.Duration(x)*time.Second), // Give some time for JS to execute - first 1 second, then 2, etc...
			chromedp.InnerHTML("*", &HtmlBody, chromedp.ByQueryAll),
		)
		...
		if bytes.Equal([]byte(HtmlBody), []byte(HtmlBodyCompare)) { //The moment
			break
		}
...
func SortRawHtml(rawHtml string, resourceType string, logTime string, countResourceTypes int) HtmlObject { //See the struct type definitions towards the top of the file
	...
	htmlObject := HtmlObject{
		...
		CountOfArmResourceTypes: countResourceTypes,
	}
...
func CompileTerraformObjects(armBasicObjects []ArmObject, htmlObjects []HtmlObject, seperateArmResources bool) []CompileObject {
	...
	//fmt.Println(htmlObjects, len(htmlObjects))
	...
	for _, armBasicObject := range armBasicObjects {
		...
		for _, armResourceType := range allResourceTypes {
			...
			if seperateArmResources {
				...
				if armResourceType.Resource_type == "Microsoft.Resources/resourceGroups" {
					rootAttribute := RootAttribute{
						Name:      "name",
						Value:     armBasicObject.Name,
						BlockName: "root",
					}
					blockAttribute := BlockAttribute{
						BlockName:     "root",
						RootAttribute: []RootAttribute{rootAttribute},
						Parent:        "root",
					}
					blockObjectsRootResources = append(blockObjectsRootResources, blockAttribute)
				}
				...
				if armResourceType.Parent == "root" {
					for _, innerArmResourceType := range allResourceTypes {
						if strings.Contains(innerArmResourceType.Resource_type, armResourceType.Resource_type) && armResourceType.Resource_type != innerArmResourceType.Resource_type && innerArmResourceType.CanBeSeperated && len(blockObjectsRootResources) == 0 {
					...
					if len(blockObjectsRootResources) > 0 {
						if armResourceType.Resource_type == "Microsoft.Resources/resourceGroups" {
							...
							htmlCapture = htmlObjectCaptureRootArmResources
						}
		...
		/*
			...
			for _, compiledObject := range compiledObjects {
				if compiledObject.ResourceName == "ruc-we-corenetwork-vnet01-to-ruc-we-ipaasintegrations-peer" {
	...
	for index, block := range blocksWithPoisonRemoved {
		for index2, rootAttribute := range block.RootAttribute {
			if CheckForEmptyArmValue(rootAttribute.Value) && !rootAttribute.IsBlock {
				for _, attribute := range htmlObject.Attribute {
					if rootAttribute.Name == attribute.Name {
						blocksWithPoisonRemoved[index].RootAttribute[index2].Value = fmt.Sprintf("value from ARM NOT FOUND BUT REQUIRED, see link for help (TOP OF THE PAGE) => \\%s", attribute.TerraformURL)
						break
					}
				}
			}
	...
	for index, block := range blocksWithPoisonRemoved {
		...
		for index2, rootAttribute := range block.RootAttribute {
			if CheckForString(rootAttribute.Value) {
				//htmlAttribute := Attribute{}
				for _, attribute := range htmlObject.Attribute {
					if attribute.Type != "armObject" && attribute.Name != "name" && strings.Contains(strings.ToLower(attribute.Descriptor), "possible value") && attribute.Name == rootAttribute.Name {
						patternPossibleValues := regexp.MustCompile(`<code>(.*?)</code>`)
						allSlicePossibleValues := patternPossibleValues.FindAllStringSubmatch(attribute.Descriptor, -1)

						allPossibleValues := []string{}
						for _, sliceValue := range allSlicePossibleValues {
							if len(sliceValue) > 1 {
								allPossibleValues = append(allPossibleValues, sliceValue[1])
							}
						}

						for _, value := range allPossibleValues {
							if value == rootAttribute.Value {
								match = true
							}
						}

						if !match && len(allPossibleValues) > 0 {
							match = false
							if len(allPossibleValues) == 1 {
								blocksWithPoisonRemoved[index].RootAttribute[index2].Value = allPossibleValues[0]
							} else {
								blocksWithPoisonRemoved[index].RootAttribute[index2].Value = fmt.Sprintf("value from ARM not valid `%s` insted replace with one of the following values: [`%s`]", rootAttribute.Value, strings.Join(allPossibleValues, "`,`"))
							}
						}
					}
				}
