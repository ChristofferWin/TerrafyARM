...
func main() {
	...
	providerVersion := flag.String("provider-version", "latest", "(Optional) Use a custom version for the terraform decompiler, useful in cases where ARM templates are old and where an older provider version might give better results, '<major, minor, patch>', eg. '3.11.0' ")
	...
	if *seperateDecompiledResources {
		log.Fatalf("The following flag 'seperate-nested-resources' is NOT in use as of version: %s", currentVersion)
		return
	}

	if *seperateSubResources {
		log.Fatalf("The following flag 'seperate-output-files' is NOT in use as of version: %s", currentVersion)
	}

	if typeValue := reflect.ValueOf(*listOfSubscriptionNamedProviders); typeValue.Kind() != reflect.String {
		log.Fatalf("The value provided of '%s' for flag '-custom-terraform-provider-names' is not a string...\nUse 'TerrafyArm -help' For more details", listOfSubscriptionNamedProviders)
	}
...
func NewTerraformConfig(terraformCompiledObject CompileObject, seperatedResources bool) string {
	...
	fmt.Println("----------------------\n", terraformCompiledObject.ArmObject.Subscription_id, "\n", terraformCompiledObject.ResourceName)
	...
	for index, blockAttribute := range sortedBlockAttributes {
		...
		if blockAttribute.BlockName == "root" {
			...
			if parentBlockName != "" || nestedBlockCount > 1 {
				fmt.Println("BLOCK ATTRIBUTE:", blockAttribute.BlockName, blockAttribute.Parent, nestedBlockCount)
			...
			fmt.Println("NESTED BLOCK COUNT:", nestedBlockCount, blockAttribute.BlockName)
		} else if blockAttribute.UniqueBlockName != "" && blockAttribute.BlockName != "root" {
			match := false
			...
			rootTerraformConfig = append(rootTerraformConfig, terraformObjectString)
			fmt.Println(len(sortedRootAttributes), "config:", rootTerraformConfig)

			if blockAttribute.Parent == "root" && nestedBlockCount > 1 {
				nestedBlockCount++
			}

			if nestedBlockCount > captureLastNestedBlocks {
				parentBlockName = blockAttribute.BlockName
				captureLastNestedBlocks = nestedBlockCount
			}

			if nestedBlockCount == 1 {
				fmt.Println("HERE WE ARE:::", nestedBlockCount, captureLastNestedBlocks, blockAttribute.BlockName, blockAttribute.Parent)
				returnRootTerraformConfig = append(returnRootTerraformConfig, strings.Repeat("\n}", captureLastNestedBlocks))
			}
	...
	fmt.Println("THIS IS IT33:-------------------------------", terraformCompiledObject.ResourceName, "\n")
	for _, line := range rootTerraformConfig {
		fmt.Println(line)
	}
	...
	for _, terraformConfigBlob := range rootTerraformConfig {
		if strings.Contains(terraformConfigBlob, "{") {
			countOfOpeningBrackets++
		} else if strings.Contains(terraformConfigBlob, "}") {
			countOfClosingBrackets++
		}
		returnRootTerraformConfig = append(returnRootTerraformConfig, terraformConfigBlob)
	}
	if countOfOpeningBrackets > 1 {
		if difference := countOfOpeningBrackets - countOfClosingBrackets; difference == 2 {
			returnRootTerraformConfig = append(returnRootTerraformConfig, "}")
		}
	} else if countOfClosingBrackets == 0 {
		returnRootTerraformConfig = append(returnRootTerraformConfig, "}")
	}

	//fmt.Println("COUNT OF OEPNINGB", countOfOpeningBrackets, countOfClosingBrackets, terraformCompiledObject.ResourceName)
...
func AddObjectTerraformAttributeForResourceDefinition(terraformBlock string, indentation string, terraformAttributes []RootAttribute, allTerraformBlocks []BlockAttribute) (string, int) {
	...
	if terraformAttributes[0].BlockName == "delegation" {
		fmt.Println("THESE ARE ALL THE ROOTS:", terraformAttributes)
	}
	...
	mapOfUniqueBlocks := make(map[string]bool)
	for _, terraformAttribute := range terraformAttributes {
		if terraformAttribute.IsBlock {
			for _, blockAttribute := range allTerraformBlocks {
				if terraformAttribute.BlockName == blockAttribute.BlockName && !mapOfUniqueBlocks[fmt.Sprintf("%s,%s", terraformAttribute.BlockName, terraformAttribute.UniqueBlockName)] {
					mapOfUniqueBlocks[fmt.Sprintf("%s,%s", terraformAttribute.BlockName, terraformAttribute.UniqueBlockName)] = true
...
func GetBlocksFromRootAttributes(rootAttributes []RootAttribute, htmlObject HtmlObject, isSeperated bool) []BlockAttribute {
	...
	*/
	...
	/*
		...
		fmt.Println("\n\n\n\n------------------------OBJECT--------------------------")
		for index, blockAttribute := range blocksWithPoisonRemoved {
			fmt.Println(index, "----------------BLOfCK---------------", blockAttribute.BlockName, "PARENT", blockAttribute.Parent, blockAttribute.UniqueBlockName)
